/**
 * Created by wujincun on 2016/5/17.
 */
(function ($) {
...
var rotatePanPage = {
    dRotate: false,
   /* obj: {
        url: './mock/coupon.json',//调取的接口连接
        failId: 5,//‘谢谢’在页面上的位置：逆时针，从0开始
        piecesNum: 8,//大转盘分为几块
        $rotate: $('#rotate'),//旋转的元素
        $targetEl: $('.lucky-draw'),//点击旋转的元素
        $closeBtn: null,//弹出页关闭按钮
        $couponHintEle: null,//中奖提示信息元素
        $ruleBtn: null,//活动规则按钮
        $mask: null,//遮罩层
        pages: {//里内容可选
            $remindPage: null,//提示页面，如‘只能抽一次’
            $failGetCouponPage: null,//未中奖页面、谢谢页面
            $couponPage: null,//中奖页
            $rulePage: null//介绍规则页
        }
    },*/
    bind: function (obj) {
        var Obj = $.extend(this.obj, obj);
        var me = this;
        Obj.$targetEl.on('click', function (e) {
            e.preventDefault();
            if (me.dRotate) {
                return false;
            } else {
        ...
        if (Obj.$closeBtn) {
            Obj.$closeBtn.on('click', function () {
                me.hideMaskPage(Obj.$mask, Obj.pages)
            });
        }
        if (Obj.$ruleBtn) {
            Obj.$ruleBtn.on('click', function () {
                me.showMaskPage(Obj.$mask, Obj.pages.$rulePage)
            })
        }
    },
    rotateInfo: function (Obj) {
        var me = this;
        $.ajax({
            url: Obj.url,
            success: function (data) {
                if (data.code == 200) {
                    ...
                    me.rotateFn(result.couponId, result.coupon, Obj);
                } else {
                    me.showMaskPage(Obj.$mask, Obj.pages.$remindPage);
                }
            }
        });
    },
    rotateFn: function (rotateDeg, msg, Obj) {
        this.dRotate = true;
        var me = this;
        Obj.$rotate.rotate({
            angle: 0,
            animateTo: 1440 + ((360 / Obj.piecesNum ) * rotateDeg) + (360 / (Obj.piecesNum * 2) ),
            duration: 8000,
            callback: function () {
                me.dRotate = false;
                if (rotateDeg == Obj.failId) {//谢谢参与
                    me.showMaskPage(Obj.$mask, Obj.pages.$failGetCouponPage);
                } else {
                    me.showMaskPage(Obj.$mask, Obj.pages.$couponPage);
                    if (Obj.$couponHintEle) {
                        Obj.$couponHintEle.text(msg)
                    }
                }
            }
        });
    },
    showMaskPage: function ($mask, $ele) {
        if ($ele) {
            $ele.css('display', 'block');
            $mask.css('display', 'block')
        }
    },
    hideMaskPage: function ($mask, pages) {
